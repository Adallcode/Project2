...
Mesh::Mesh()
	...
	, mRadius(0.0f)
	, mSpecPower(100.0f),
	mBox(Vector3(Math::Infinity, Math::Infinity, Math::Infinity), Vector3(Math::NegInfinity, Math::NegInfinity, Math::NegInfinity))
{
}
...
bool Mesh::Load(const std::string & fileName, Renderer* renderer)
{
	...
	for (rapidjson::SizeType i = 0; i < vertsJson.Size(); i++)
	{
		...
		Vector3 pos((float) vert[0].GetDouble(), (float) vert[1].GetDouble(), (float) vert[2].GetDouble());
	...
	mRadius = Math::SquareRoot(mRadius);
